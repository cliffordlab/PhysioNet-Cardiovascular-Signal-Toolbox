...
signForce = []; % if we want to force the sign of the peak we are looking for
...
%%%%%%%%%%%%                [QRStemp,signForce] = qrs_detect2(ecg(start-dTminus:stop+dTplus),0.150,thresTrans,fs,[],signForce);
                [QRStemp,signForce] = qrs_detect3(ecg(start-dTminus:stop+dTplus),0.150,thresTrans,fs);

                thresTrans = thresTrans-0.1;
            end
        else
%%%%%%%%            [QRStemp,signForce] = qrs_detect2(ecg(start-dTminus:stop+dTplus),0.250,thres,fs,[],signForce);
            [QRStemp,signForce] = qrs_detect3(ecg(start-dTminus:stop+dTplus),0.250,thres,fs);
        end
