function [ NTWDseries TWDseries ] = DiffSeriesComplexesCalc( Complexes )
...
for index = 1:size(Complexes,1)-1
    ...
    complex1 = Complexes(index,:);    % signalnormalized(rpoints(index) - medianonset:rpoints(index) + medianoffset);
    complex2 = Complexes(index+1,:);    % signalnormalized(rpoints(index+1) - medianonset:rpoints(index+1) + medianoffset);
