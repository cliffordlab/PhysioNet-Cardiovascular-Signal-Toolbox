...
onset.
  ...
  08/03/2010: A few modifications by Qiao Li:
  //            1)  sample() function sometimes get WFDB_INVALID_SAMPLE value even when data is good.
  //                Changed to mysample(), using isigsettime() and getvec() instead, but slow!!!
  1)    Add a mysetbuf() function, read the data to a databuf first, then mysample()
  get data from the buf. It's much more fast than before.

  Waiting for George for latest version of sample()...
...
int dataend=0;
WFDB_Sample databuf[BUFLN2]={0};
long databufp;
long dataL=0;

int mysetbuf(WFDB_Signal s,WFDB_Time t) {

  WFDB_Sample v[WFDB_MAXSIG];

  while(dataL<=t) {
    if (getvec(v)>0) {
      if (v[s]>WFDB_INVALID_SAMPLE+10)
        databuf[databufp]=v[s];
      else
        databuf[databufp]=0;
      databufp++;
      if (databufp>=BUFLN2)
...
// Change sample to mysample temporarily.

WFDB_Sample mysample(WFDB_Signal s, WFDB_Time t)
{
  long point1;

  if (mysetbuf(s,t)>0) {
    point1=dataL-t-1;
    point1=((databufp-1)-(dataL-t-1)+BUFLN2)%BUFLN2;
    return databuf[point1];
  }
  else {
    dataend=1;
...
WFDB_Sample slpsamp(WFDB_Time t) {
	...
	int val1;
	int val2;
	val2=mysample(sig, tt - 1);
	if(dataend)
		val2=prevVal;
	val1=mysample(sig, tt);
	if(dataend)
		val1=val2;
	prevVal=val2;
        dy =  val1-val2;
        //dy = 10 * dy; //make applicable for PLETH
        if (dy < 0) dy = 0;
...
{ 
    ...
    WFDB_Siginfo *s;
    WFDB_Time from = 0L, next_minute, spm, t, tpq, to = 0L, tt, t1,tmax,maxmin_2_3_threshold;
    char *p, *prog_name();
    static int gvmode = 0;
    void help();
    char *annot_name = NULL;
	...
	if (*argv[i] == '-') switch (*(argv[i]+1)) {
	  ...
	  case 'm':	/* threshold */
	    ...
	    break;
	  case 'o':	/* annotation name */
	    if (++i >= argc) {
		(void)fprintf(stderr, "%s: annotation name must follow -o\n", 
			      pname);
		exit(1);
	    }
	    annot_name = argv[i];
...
printf("Processing %s ... \n",record);
    ...
    if (sig < 0 || sig >= nsig) {
	/* Identify the lowest-numbered PLETH signal */
	for (i = 0; i < nsig; i++)
            if (strcmp(trim_whitespace(s[i].desc), "PLETH") == 0 ||
                    strcmp(s[i].desc, "PPG") == 0)
 		break;
	if (i == nsig) {
	    fprintf(stderr, "%s: no PLETH signal specified; use -s option\n\n",
		    pname);
	    help();
	    exit(3);
	}
...
//	    printf("%6d\t%6d\n", sample(sig, t), slpsamp(t));
	...
	if (slpsamp(t) > T1) {   /* found a possible ABP pulse near t */ 
	    timer = 0; 
            ...
            if ((max == slpsamp(t)))
                continue;
            for (tt = tmax; tt > t - EyeClosing / 2; tt--)
                if (slpsamp(tt) < min) min = slpsamp(tt);
            if (max > min + 10) {
                onset = (max-min) / 100 + 2;
                tpq = t - 5;
                maxmin_2_3_threshold=(max-min)*2.0/3;
                for (tt=tmax;tt>t-EyeClosing/2;tt--)
                    if (slpsamp(tt)<maxmin_2_3_threshold)
                            break;
                for (tt; tt > t - EyeClosing / 2 +2; tt--) {
                    if (slpsamp(tt) - slpsamp(tt - 3) < onset) {
                        tpq = tt-2;
                        break;
                    }
                }
...
static char *help_strings[] = {
 ...
 " -o          name the annotation file suffix for output (default: wpleth)", 
