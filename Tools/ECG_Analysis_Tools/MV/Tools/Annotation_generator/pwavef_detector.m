function     [position,picon_all,picoff_all,messages]= pwavef_detector(heasig,samp,time,position,w,intervalo,ultimo_anot,messages)
...
%
% Rute Almeida 
% based on pwave.m by Juan Pablo Mart�nez Cort�s
    ...
    for i = 1:length(unique(intervalo(1):intervalo(end)))
       ...
       finvent = round(messages.setup.wavedet.finvent_tol_P*messages.setup.wavedet.freq);
        ...
        if endwin-begwin>=messages.setup.wavedet.min_vent_P*messages.setup.wavedet.freq; %27SEP2011
            ...
            pwave_task1_detector % subtask finding extreme points and deciding if there is wave is the scale n
