function     [position,qrspiconset,qrspicoffset,messages]= qrswavef_detector(heasig,samp,time,position,w,intervalo,messages)
...
for i = 1:length(time), 
    ...
    if (sign(apa)==sign(app)),        % In that case...
        ...
        % and what happens if there is no peak ??  Solucionado esto �ltimo
    end
    ...
    if (apa>0 & app<0),   %#ok<AND2> % qrs is positive qRs, Rsr, rsR.
        ...
        if ~isempty(zp),
            ...
            %  Protection against waves with cracks
            paux =modmax(w(ppostzp+1:min(size(w,1),ppostzp+round(QRS_wtol_ppostzp_crack*(messages.setup.wavedet.freq))),2),2,umbral2p*dermax,sign(w(ppostzp,2)));  % estamos a ir longe de mais na busca falta protec�ao para que nao entre no batimento seguinte!
            if ~isempty(paux)
                paux = ppostzp + paux(end);
                ppostzp = paux;
                appostzp = sign(w(paux,2))*max(abs(appostzp), abs(w(paux,2)));
