...
int check_for_ectopic(int noEctopic)
{
...
int check_for_noise(int state, float hr, int noNoise)
{
  int rtn=NO; 
...
void initialize(long seed, long tmax, int noEctopic, int noNoise)
{
   ...
   */
   ...
   for(i=2;i<=Nbeats;i++){
     ...
     test = check_for_ectopic(noEctopic); /* no state dependency */
     if (test!=NO){
       trpeaks_new[j] = modify_ectopic(trpeaks[i],trpeaks[i-1]);
     }    
     if (test == NO){ /* just keep the current beat */
       test = check_for_noise(state[i], hr, noNoise); /* hr and state dependent */
...
int main(int argc, char **argv)
{
    ...
    int noEctopic = YES;    /* do we want ectopic beats ? */
    int noNoise   = YES;    /* do we want ectopic noise ? */
    long atol();

    if (argc < 2) {
	fprintf(stderr, "usage: %s seed [tmax] noEctopics? noNoise?\n", argv[0]);
	exit(1);
    }
    seed = atol(argv[1]);
    ...
    /* This can be a zero or a 1*/
    noEctopic = atoi(argv[3]);

    /* This can be a zero or a 1*/
    noNoise = atoi(argv[4]);

    initialize(seed, tmax, noEctopic, noNoise);
